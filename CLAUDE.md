...
- Web tools live in `app/agent/web_tools.py`: `web_search` (DuckDuckGo via `ddgs`, no API key) and `fetch_url` (reader-mode extraction via `trafilatura`). Both run their blocking I/O in `asyncio.to_thread`. `fetch_url` output is truncated at 8000 chars; errors become the tool result so the agent can recover rather than crashing the turn. (Originally used `duckduckgo-search` but its search went dead — the package was renamed to `ddgs`.)
